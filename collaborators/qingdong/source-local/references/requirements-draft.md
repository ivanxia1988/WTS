...
主 Agent 在步骤 5、步骤 9 改尺度和检索中途补充需求时读。确认稿是给 Agent 的规格，写成 JSON 文件；画像是确认稿的白话投影，展示给用户。Agent 只编辑确认稿，画像每次都从确认稿重新渲染。

## 确认稿文件

路径：`<TASK_WORK_DIR>/wts-local/requirements/vN.json`，N 从 1 起，每次修改写新的 N，旧版本保留。**当前需求版本**就是 N 最大的那份。
...
| `position` | 【岗位】 | `{title, primary_anchor, secondary_anchor, anchor_source: "title" \| "jd_summary", level_years}`；锚点照简历写法，含空格时 ledger.py 按空格拆成几个词，每条查询都要全部带上 |
| `mission` | 【岗位使命】 | 一句话 |
...
| `keywords` | 【检索关键词】 | `{active: [{word, family}], reserve: [{word, family}]}`，active ≤6、reserve ≤2；`word` 不含空格 |
...
ledger.py 从这个文件读 `position.primary_anchor` 校验查询，读 `must_have`、`nice_to_have`、`exclude_signals` 决定总分里哪些维度为 null，所以这三个键即使为空也写成 `[]`。
